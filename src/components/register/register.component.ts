        ...
        <div class="bg-white/20 dark:bg-gray-800/20 backdrop-blur-xl border border-white/30 dark:border-gray-700/30 rounded-2xl shadow-2xl p-8 text-white">
          ...
          <form (ngSubmit)="onSubmit()" #registerForm="ngForm">
            ...
            <div class="mb-4">
              ...
              <div class="relative">
                <input [type]="confirmPasswordVisible() ? 'text' : 'password'" id="confirmPassword" name="confirmPassword" [(ngModel)]="confirmPassword" required
                  class="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-lg focus:ring-2 focus:ring-white/50 focus:outline-none placeholder-gray-400 transition pr-10"
                  placeholder="••••••••">
                <button type="button" (click)="toggleConfirmPasswordVisibility()" class="absolute inset-y-0 right-0 flex items-center px-3 text-gray-300 hover:text-white">
            ...
            <button type="submit" [disabled]="isLoading() || registerForm.invalid || password !== confirmPassword"
